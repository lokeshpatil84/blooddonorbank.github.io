...
class DBConnect {
    ...
    public function authLogin(){
        session_start();
        if(isset($_SESSION['username'])){
            header("Location: http://localhost/bloodbank/home.php");
        }
    }
    ...
    public function addDonor($fname,$mname,$lname,$sex,$bType,$dob,$hAddress,$city,$mobile){
        $stmt = $this->db->prepare("INSERT INTO donors (fname,mname,lname,sex,b_type,bday,h_address,city,mobile)"
                . "VALUES (?,?,?,?,?,?,?,?,?)");
         $stmt->execute([$fname,$mname,$lname,$sex,$bType,$dob,$hAddress,$city,$mobile]);
        return true;
